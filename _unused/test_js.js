        ...
        function refresh() {
            ...
            api('/api/status?hours=' + filterHours).then(function(data) {
                ...
                if (btnAuto) {
                    if (autoRunning) {
                        btnAuto.innerHTML = '<span id="autoIcon">&#x23F8;</span> <span id="autoText">PAUSE</span>';
                        btnAuto.classList.add('on');
                    } else {
                        btnAuto.innerHTML = '<span id="autoIcon">&#x25B6;</span> <span id="autoText">PLAY</span>';
                ...
                if (allSignals.length === 0) {
                    setHTML('signals', '<div class="no-data">No signals found. Select pairs above and click SCAN NOW. Signals appear when H4 and H1 trends align (both BULLISH or both BEARISH).</div>');
                } else {
                    var sigHTML = '';
                ...
                allHistory = data.trade_history || [];
                currentPage = 1;
                renderHistory();
                
                log('Refresh complete - balance: ' + data.balance);
            });
        }
        ...
        function renderHistory() {
            ...
            var totalPages = Math.ceil(allHistory.length / itemsPerPage);
            var start = (currentPage - 1) * itemsPerPage;
            var end = start + itemsPerPage;
            var pageItems = allHistory.slice(start, end);
            ...
            var pageHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;">' +
                '<span style="color:#8b949e;font-size:11px;">Showing ' + (start + 1) + '-' + Math.min(end, allHistory.length) + ' of ' + allHistory.length + ' trades</span>' +
                '<div style="display:flex;gap:5px;">';
            
            if (currentPage > 1) {
                pageHTML += '<button class="btn" style="padding:5px 10px;background:#21262d;color:#c9d1d9;border:1px solid #30363d;" onclick="changePage(-1)">Prev</button>';
            }
            pageHTML += '<span style="padding:5px 10px;color:#8b949e;">Page ' + currentPage + '/' + totalPages + '</span>';
            if (currentPage < totalPages) {
                pageHTML += '<button class="btn" style="padding:5px 10px;background:#21262d;color:#c9d1d9;border:1px solid #30363d;" onclick="changePage(1)">Next</button>';
            }
            pageHTML += '</div></div>';
            
            setHTML('history', histHTML);
            setHTML('historyPagination', pageHTML);
        }
        ...
        function doScan() {
            if (selectedPairs.length === 0) {
                setHTML('signals', '<div class="no-data" style="color:#f85149;">Please select at least one pair to scan first!</div>');
                return;
            }
            ...
            setHTML('signals', '<div class="no-data" style="color:#58a6ff;">Scanning ' + selectedPairs.join(', ') + '...</div>');
            api('/api/scan?pairs=' + selectedPairs.join(','), 'POST').then(function(d) { 
                refresh();
                if (btn) {
                    btn.disabled = false;
        ...
        function executeAction() {
            ...
            if (pendingAction === 'close-profitable') {
                api('/api/close-profitable', 'POST').then(function() { refresh(); });
            } else if (pendingAction === 'close-all') {
                api('/api/close-all', 'POST').then(function() { refresh(); });
            } else if (pendingAction.startsWith('close-pos-')) {
                var ticket = pendingAction.replace('close-pos-', '');
                api('/api/close/' + ticket, 'POST').then(function() { refresh(); });
            }
        }
        
        function closePos(ticket) {
            confirmAction('close-pos-' + ticket, 'Close position #' + ticket + '?');
        }
